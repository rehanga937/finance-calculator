...
class MortgageHistoryViewTableViewController: UITableViewController {
    
    var array = [mortgageRecord]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var yearVersion = array[array.count - 1 - indexPath.row].a
        yearVersion = (((yearVersion/12)*100).rounded())/100 //since mortgage is an annual granularity loan but with payments calculated monthly
        
        cell!.DisplayLabel.text = "\(array[array.count - 1 - indexPath.row].notes)\nMortgage Value: \(array[array.count - 1 - indexPath.row].x)\nInterest Rate p.a.: \(array[array.count - 1 - indexPath.row].m)%\nMonthly Payment Value: \(yearVersion)\nDuration in months: \(array[array.count - 1 - indexPath.row].n)"
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?){
        ...
        case "HistoryToAmortize_Mortgage":
            ...
            destVC.a = (selectedRecord.a)/12 //since mortgage is an annual granularity loan but with payments calculated monthly
